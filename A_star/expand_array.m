function exp_array=expand_array(node_x,node_y,gn,xTarget,yTarget,CLOSED,MAX_X,MAX_Y,xStart,yStart)
            ...
            if (k~=j || k~=0)  %The node itself is not its successor
                ...
                if( (s_x >0 && s_x <=MAX_X) && (s_y >0 && s_y <=MAX_Y))%node within array bound
                    ...
                    if (flag == 1)
                        ...
                        cross = abs(dx1*dy2-dx2*dy1);%相当于叉乘，它的值代表的含义是更倾向于沿着起点到终点的连线走（直线）sin（seita）=0
                        %加入h=h+cross*0.001,
                        exp_array(exp_count,3) = distance(xTarget,yTarget,s_x,s_y)+cross*0.001; %distance between node and goal,hn
                        %exp_array(exp_count,3) = distance(xTarget,yTarget,s_x,s_y);
                        exp_array(exp_count,4) = gn+distance(node_x,node_y,s_x,s_y);%cost of travelling to node，gn是累加和
